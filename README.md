...
| `FEATURE_TRACKER_DESCRIPTION_MODE` | `structured` | Description format: `structured` (object with outcome + optional problem/notes/tags) or `simple` (string outcome only) |
...
| `FEATURE_TRACKER_MD_EXPORT_LAYOUT` | `flat` | Folder layout for exported `.md` files: `flat` (all files at the root of the export dir) or `nested` (`<project>/<YYYY-MM>/<file>.md`). The `nested` layout scales better when features accumulate. |
...
**`structured`** (default) — object with up to four fields:
...
Only `outcome` is required; Claude omits the other fields if the conversation doesn't have clear content for them.

**`simple`** — single string with the outcome:
...
**Folder layout** is controlled by `FEATURE_TRACKER_MD_EXPORT_LAYOUT`:

- `flat` (default) — every file lives directly under the export dir.
  ```
  feature-tracker/
  ├── 2026-04-15-login-fix.md
  ├── 2026-05-01-properties-redesign.md
  └── 2026-05-02-platform-support.md
  ```
- `nested` — files are bucketed by project slug + year-month. Recommended once you have more than a few dozen features.
  ```
  feature-tracker/
  ├── kairo-crm/
  │   ├── 2026-04/
...
To switch layouts, change the env var and run `/feature-tracker:export-md` — it removes any tracker-managed files (those matching `YYYY-MM-DD-*.md`, recursively) and regenerates them in the new layout. User-created notes (e.g. a `_index.md` Dataview dashboard) are left untouched. Dataview queries that target the export folder (`FROM "feature-tracker"`) work in both layouts because subfolders are included automatically.
